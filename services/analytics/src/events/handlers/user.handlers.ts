import { ADMIN_EVENTS, AUTH_EVENTS } from "@leetconnect/shared";
	...
	try {
		...
		await prisma.user.upsert({
			...
			update: {
				...
				lastname: lastname, status: status, role: role, email: email, type: type
			},
			create: {
...
export async function handleUserUpdatedAdmin(channel: string, message: any): Promise<void> {
	if (channel !== ADMIN_EVENTS.USER_UPDATED) return;
	try {
...
export async function handleUserDeleted(channel: string, message: any): Promise<void> {
	if (channel !== ADMIN_EVENTS.USER_DELETED) return;
	try {
